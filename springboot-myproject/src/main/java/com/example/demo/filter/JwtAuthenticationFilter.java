...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		System.out.println("🔥 Filter running for: " + method + " " + path);
		...
		if (authHeader != null && authHeader.startsWith("Bearer ")) {
			...
			try {
				...
				UserCertDto userCertDto = new UserCertDto(userId, username, role, token);
				List<GrantedAuthority> authorities = List.of(new SimpleGrantedAuthority("ROLE_" + role));
				UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
						userCertDto, null, authorities);

				SecurityContextHolder.getContext().setAuthentication(authentication);
				request.setAttribute("userCertDto", userCertDto); // 可選

				filterChain.doFilter(request, response);
			} catch (Exception e) {
				System.out.println("❌ Token 錯誤：" + e.getMessage());
